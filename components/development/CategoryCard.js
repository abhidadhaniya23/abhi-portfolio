...
const CategoryCard = ({ index, name, icon, projects, setFilteredProjects, selectedCategory, id, setSelectedCategory }) => {
    const animations = {
        card: {
            initial: { opacity: 0, y: 20 },
            exit: { opacity: 0, y: 10, transition: { duration: 0.15, delay: index * 0.05 } },
            animate: { opacity: 1, y: 0, transition: { duration: 0.3, delay: index * 0.1 + 0.2 } },
        },
    };
    useEffect(() => {
        const filteredProjectsArr = projects.filter((project) => project.categoryId.includes(selectedCategory));
